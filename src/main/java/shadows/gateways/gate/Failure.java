...
public interface Failure {
	...
	public static void initSerializers() {
		SERIALIZERS.put("explosion", new SerializerBuilder<ExplosionFailure>("Explosion Failure").autoRegister(ExplosionFailure.class).build(true));
		SERIALIZERS.put("mob_effect", new SerializerBuilder<MobEffectFailure>("Mob Effect Failure").autoRegister(MobEffectFailure.class).build(true));
		SERIALIZERS.put("summon", new SerializerBuilder<SummonFailure>("Summon Failure").autoRegister(SummonFailure.class).build(true));
		SERIALIZERS.put("chanced", new SerializerBuilder<ChancedFailure>("Chanced Failure").autoRegister(ChancedFailure.class).build(true));
		SERIALIZERS.put("command", new SerializerBuilder<CommandFailure>("Command Failure").autoRegister(CommandFailure.class).build(true));
	}
	...
	public static record ExplosionFailure(float strength, boolean fire, boolean blockDamage) implements Failure {
		...
		@Override
		public void appendHoverText(Consumer<Component> list) {
			list.accept(new TranslatableComponent("failure.gateways.explosion", this.strength, this.fire, this.blockDamage));
		}
	}
	...
	public static record MobEffectFailure(MobEffect effect, int duration, int amplifier) implements Failure {
		...
		@Override
		public void appendHoverText(Consumer<Component> list) {
			list.accept(new TranslatableComponent("failure.gateways.mob_effect", toComponent(new MobEffectInstance(effect, duration, amplifier))));
		}
	...
	public static record SummonFailure(EntityType<?> type, @Nullable CompoundTag nbt, int count) implements Failure {
		...
		public JsonObject write() {
			JsonObject obj = Failure.super.write();
			obj.addProperty("entity", ForgeRegistries.ENTITIES.getKey(type).toString());
		...
		@Override
		public void appendHoverText(Consumer<Component> list) {
			list.accept(new TranslatableComponent("failure.gateways.summon", count, new TranslatableComponent(type.getDescriptionId())));
		}
	}
	...
	public static record ChancedFailure(Failure failure, float chance) implements Failure {
		...
		public void appendHoverText(Consumer<Component> list) {
			this.failure.appendHoverText(c -> {
				list.accept(new TranslatableComponent("failure.gateways.chance", fmt.format(chance * 100), c));
			});
		}
	}
