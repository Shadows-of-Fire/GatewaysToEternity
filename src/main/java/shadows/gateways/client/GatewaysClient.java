...
public class GatewaysClient {
	...
	public static void scroll(ScreenEvent.MouseScrollEvent.Pre e) {
		if (currentTooltipItem.getItem() == GatewayObjects.GATE_PEARL && tooltipTick == PlaceboClient.ticks && Screen.hasShiftDown()) {
	...
	public static void scroll2(InputEvent.MouseScrollEvent e) {
		if (currentTooltipItem.getItem() == GatewayObjects.GATE_PEARL && tooltipTick == PlaceboClient.ticks && Screen.hasShiftDown()) {
			waveIdx += e.getScrollDelta() < 0 ? 1 : -1;
			e.setCanceled(true);
		} else {
	...
	public static void tooltip(ItemTooltipEvent e) {
		...
		if (e.getItemStack().getItem() == GatewayObjects.GATE_PEARL) {
			...
			if (Screen.hasShiftDown()) {
				...
				int wave = waveIdx;
				Component sub = new TranslatableComponent("tooltip.gateways.scroll").withStyle(Style.EMPTY.withColor(ChatFormatting.DARK_GRAY).withItalic(true).withUnderlined(false));
				comp = new TranslatableComponent("tooltip.gateways.wave", wave + 1, gate.getNumWaves(), sub).withStyle(ChatFormatting.GREEN, ChatFormatting.UNDERLINE);
				tooltips.add(comp);
			...
			if (Screen.hasControlDown()) {
				comp = new TranslatableComponent("tooltip.gateways.completion").withStyle(Style.EMPTY.withColor(0xFCFF00).withUnderlined(true));
				tooltips.add(comp);
				tooltips.add(Component.nullToEmpty(null));
				comp = new TranslatableComponent("tooltip.gateways.experience", gate.getCompletionXp()).withStyle(Style.EMPTY.withColor(0xFCFF00));
				tooltips.add(comp);
				for (Reward r : gate.getRewards()) {
					r.appendHoverText(c -> {
						tooltips.add(new TranslatableComponent("tooltip.gateways.list2", c).withStyle(Style.EMPTY.withColor(0xFCFF00)));
					});
				}
			} else {
