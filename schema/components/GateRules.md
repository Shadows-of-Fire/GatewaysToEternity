...
```js
{
    ...
    "follow_range_boost": float,       // [Optional] || Bonus follow range added to all entities spawned by this gateway. Not shown to users. Default value = 32.
    ...
    "requires_nearby_player": boolean, // [Optional] || If true, the gateway will fail if 10 seconds pass while no player is inside the leash range. Default value = false.
    "lives": integer                   // [Optional] || If set to a positive number, this is the number of player deaths that are allowed before the gateway fails. Default value = -1 (infinite lives). Range: [-1, 1024].
}
```